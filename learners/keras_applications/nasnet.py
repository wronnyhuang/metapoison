...
def NASNet(input_shape=None,
           ...
           pooling=None,
           classes=1000,
           default_size=None, params=PARAM_NONE,
           **kwargs):
    ...
    x = layers.Conv2D(stem_block_filters, (3, 3),
                      ...
                      use_bias=False,
                      name='stem_conv1',
                      kernel_initializer='he_normal')(img_input, params=params)

    x = layers.BatchNormalization(
        axis=channel_dim, momentum=0.9997, epsilon=1e-3, name='stem_bn1')(x, params=params)

    p = None
    x, p = _reduction_a_cell(x, p, filters // (filter_multiplier ** 2), params=params,
                             block_id='stem_1')
    x, p = _reduction_a_cell(x, p, filters // filter_multiplier, params=params,
                             block_id='stem_2')

    for i in range(num_blocks):
        x, p = _normal_a_cell(x, p, filters, params=params, block_id='%d' % (i))

    x, p0 = _reduction_a_cell(x, p, filters * filter_multiplier, params=params,
                              block_id='reduce_%d' % (num_blocks))

    p = p0 if not skip_reduction else p

    for i in range(num_blocks):
        x, p = _normal_a_cell(x, p, filters * filter_multiplier, params=params,
                              block_id='%d' % (num_blocks + i + 1))

    x, p0 = _reduction_a_cell(x, p, filters * filter_multiplier ** 2, params=params,
                              block_id='reduce_%d' % (2 * num_blocks))

    p = p0 if not skip_reduction else p

    for i in range(num_blocks):
        x, p = _normal_a_cell(x, p, filters * filter_multiplier ** 2, params=params,
...
def NASNetLarge(input_shape=None,
                ...
                input_tensor=None,
                pooling=None,
                classes=1000, params=PARAM_NONE,
                **kwargs):
...
def _separable_conv_block(ip, filters,
    ...
    x = layers.SeparableConv2D(filters, kernel_size,
                               ...
                               kernel_initializer='he_normal')(x, params=params)
    x = layers.BatchNormalization(
        axis=channel_dim,
        momentum=0.9997,
        epsilon=1e-3,
        name='separable_conv_1_bn_%s' % (block_id))(x, params=params)
    x = layers.Activation('relu')(x)
    x = layers.SeparableConv2D(filters, kernel_size,
    ...
    x = layers.BatchNormalization(
        ...
        momentum=0.9997,
        epsilon=1e-3,
        name='separable_conv_2_bn_%s' % (block_id))(x, params=params)
    return x


def _adjust_block(p, ip, filters, params=PARAM_NONE, block_id=None):
    ...
    if p is None:
        ...
        p1 = layers.Conv2D(
            ...
            kernel_initializer='he_normal')(p1, params=params)
        ...
        p2 = layers.Conv2D(
            ...
            kernel_initializer='he_normal')(p2, params=params)
        ...
        p = layers.BatchNormalization(
            ...
            momentum=0.9997,
            epsilon=1e-3,
            name='adjust_bn_%s' % block_id)(p, params=params)
        ...
        p = layers.Conv2D(
            ...
            use_bias=False,
            kernel_initializer='he_normal')(p, params=params)
        p = layers.BatchNormalization(
            axis=channel_dim,
            momentum=0.9997,
            epsilon=1e-3,
...
def _normal_a_cell(ip, p, filters, params=PARAM_NONE, block_id=None):
    ...
    p = _adjust_block(p, ip, filters, params=params, block_id=block_id)
    ...
    x5 = _separable_conv_block(h, filters, params=params,
...
def _reduction_a_cell(ip, p, filters, params=PARAM_NONE, block_id=None):
    ...
    h = layers.BatchNormalization(
        ...
        momentum=0.9997,
        epsilon=1e-3,
        name='reduction_bn_1_%s' % block_id)(h, params=params)
    ...
    # with backend.name_scope('block_1'):
    x1_1 = _separable_conv_block(
        h, filters, (5, 5), params=params,
        strides=(2, 2),
        block_id='reduction_left1_%s' % block_id)
    x1_2 = _separable_conv_block(
        p, filters, (7, 7), params=params,
        strides=(2, 2),
